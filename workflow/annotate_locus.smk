...
rule run_dupmasker:
  '''call dupmasker snakemake and run with manifest generated by _get_rhodonite_manifest'''
  input:
    manifest = rules._get_rhodonite_manifest.dup_man
  output:
    bed="Dupmasker/{samp}.duplicons.bed.gz"
  wildcard_constraints:
    samp = manifest["sample"] + "_" + manifest["hap"]
  shell:"""
snakemake -s get_rhodonite.smk only_dup -j 50 -p --config manifest={input.manifest} 
"""
